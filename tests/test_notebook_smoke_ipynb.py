...
def test_ci_notebooks_config_lists_smoke_paths() -> None:
    ...
    assert "CHARTER_WIKITEXT_STREAM_SMOKE.ipynb" in txt
...
def test_charter_domain_stream_notebook_wired() -> None:
    p = Path(__file__).resolve().parents[1] / "notebooks" / "CHARTER_WIKITEXT_STREAM_SMOKE.ipynb"
    assert p.is_file(), f"missing {p}"
    nb = json.loads(p.read_text(encoding="utf-8"))
    blob_parts: list[str] = []
    for c in nb.get("cells", []):
        ...
        blob_parts.append(src)
    blob = "\n".join(blob_parts)
    assert "charter_space_wikitext_stream_smoke" in blob and "load_dataset" in blob
